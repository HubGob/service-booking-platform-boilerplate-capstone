    ...
    <div className="dashboard">
      ...
      <section className="dash-section">
        ...
        <button
          ...
          onClick={() => {
            ...
            const cat = prompt(
              'Category (tutoring, consulting, design, writing, development, marketing, music, other):',
            );
          ...
          <div className="services-list">
            {services.map((s) => (
              <div key={s._id} className="service-item">
                <strong>{s.title}</strong> — ${s.price} / {s.durationMinutes}min
                [{s.category}]
                <button
      ...
      <section className="dash-section">
        ...
        {availability.length === 0 ? (
          <p className="empty-state">No availability slots yet.</p>
        ) : (
          ...
          <div className="bookings-list">
            {bookings.map((b) => (
              <div key={b._id} className="booking-item">
                <div>
                  <strong>{b.service?.title || 'Service'}</strong> with{' '}
                  {b.client?.name || 'Client'}
                </div>
                <div className="booking-time">
                  {new Date(b.startTime).toLocaleString()} — ${b.totalPrice}
                </div>
                <button
