      ...
      try {
        ...
        if (svcRes.provider) {
          const availRes = await fetch(
            `api/availability/provider/${svcRes.provider._id}`,
          ).then((r) => r.json());
            ...
            <div className="provider-info">
              ...
              <div>
                ...
                <p className="provider-specialty">
                  {service.provider?.specialty || 'Service Provider'}
                </p>
              </div>
            </div>
                    ...
                    <>
                      <div className="slots-list">
                        ...
                        {availability.map((slot) => (
                          <label
                            key={slot._id}
                            className={`slot-option ${selectedSlot?.includes(slot.startTime) ? 'selected' : ''}`}
                          >
                            <input
                              type="radio"
                              name="slot"
                              value={`${slot.dayOfWeek} ${slot.startTime}-${slot.endTime}`}
                              onChange={(e) => setSelectedSlot(e.target.value)}
                            />
                            <span>
                              Day {slot.dayOfWeek} · {slot.startTime} -{' '}
                              {slot.endTime}
                            </span>
                          </label>
                        ))}
            ...
            {!isAuthenticated && (
              <div className="auth-prompt">
                <p>
                  ...
                  <Link to="/register">register</Link> to book this service.
                </p>
              </div>
            )}
            {user?.role !== 'client' && isAuthenticated && (
              <div className="auth-prompt">
                <p>Switch to client role to book services.</p>
              </div>
            )}
          </div>
        </>
