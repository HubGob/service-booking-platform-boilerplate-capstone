...
const ServiceDetail = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const [svcRes] = await Promise.all([fetch(`api/services/${id}`).then(r => r.json())]);
        setService(svcRes);
        if (svcRes.provider) {
  ...
  const handleBook = async () => {
    ...
    try {
      ...
      slotDate.setHours(...startTime.split(':').map(Number));
      const endSlot = new Date(slotDate);
      endSlot.setHours(...endTime.split(':').map(Number));
